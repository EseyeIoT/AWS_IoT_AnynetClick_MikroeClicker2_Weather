...
static char I2C_ReadRegister(char rAddr) {
  ...
  I2C2_Wr(tmp_data[0]);
  I2C2_Repeated_Start();
  I2C2_Wr(BME280_I2C_ADDRESS1 + 1);
  tmp_data[0] = I2C2_Rd(_I2C_NACK);
  I2C2_Stop();
  return tmp_data[0];
}
...
void BME280_ReadMeasurements() {
  ...
  for (i = 0; i < BME280_DATA_FRAME_SIZE; i++)
  {
       tmp_data[i] =  I2C2_Rd(_I2C_ACK);
  }
  tmp_data[i+1] = I2C2_Rd(_I2C_NACK);
